	...
	window.MessagesHeaderReader = { 
		...
		getHeaderValueFrom : function(aHeader, aMessageURI) 
		{
			if (this.isAttachment) {
				...
				var value = this.getHeaderValueFromRawContents(aHeader, aMessageURI);
		...
		getHeaderValueFromRawContents : function(aHeader, aMessageURI) 
		{
			var source = this.readMessageFromURI(aMessageURI, aHeader);
			return source ? this.getHeaderValueFromSource(aHeader, source) : null ;
		},
 
		getHeaderValueFromAttachment : function(aHeader) 
		{
			var contents = this.cachedContents || this.readMessageFromURI(this.messageURI.spec);
			if (!contents) return null;
			this.cachedContents = contents;
			return this.getHeaderValueFromCache(aHeader);
		},
	
		readMessageFromURI : function(aURI, aHeader) 
		{
			try {
				...
				var offset = 0;
				var part = 0;
				var match = aURI.match(this.kMULTIPART_OFFSET_REGEXP);
				if (match) {
					offset = parseInt(match[1]);

					match = aURI.match(/[?&]part=1.([0-9]+)/);
					part = match ? parseInt(match[1]) : 0 ;
				}
				else {
					match = aURI.match(this.kRAW_MESSAGE_OFFSET_REGEXP);
					if (!match) return null;
					offset = match[2];
					aURI = match[1];
					aURI = RDF.GetResource(aURI.replace(/^([^-]+)-message:/, '$1:'))
							.QueryInterface(Components.interfaces.nsIMsgFolder)
							.folderURL;
					aURI = escape(aURI).replace(escape(':'), ':');
				}

				var uri = IOService.newURI(aURI.replace(/^mailbox:/, 'file:'), null, null);

				var boundary = '';
				var content;

				var findingHeaderRegExp = aHeader ? new RegExp('^'+aHeader+':', 'i') : null ;
				var headerRegExp = /^[^:]+:/;
				...
				if (!gDBView.db) { // for attachments of local files
					...
					if (part) {
						...
						for (var i = 0; i < count; i++)
						{
							...
							if (findingHeaderRegExp && headerRegExp.test(line.value)) {
								if (headerFound) {
									break;
								}
								else {
									headerFound = findingHeaderRegExp.test(line.value);
								}
							}
							content.push(line.value);
						}
		...
		kRAW_MESSAGE_OFFSET_REGEXP : /^([^#]+)#([0-9]+)$/,
		kMULTIPART_OFFSET_REGEXP : /[?&]number=([0-9]+)/,
		kMULTIPART_PART_REGEXP : /[?&]part=1.([0-9]+)/,
		...
		getHeaderValueFromCache : function(aHeader) 
		{
			...
			this.cachedHeader[aHeader] = this.getHeaderValueFromSource(aHeader, this.cachedContents);
			return this.cachedHeader[aHeader];
		},
		cachedHeader : {},
		cachedContents : null,
 
		getHeaderValueFromSource : function(aHeader, aSource)
		{
			if (!aSource) return '';
	...
	window.ComposerRecipientsListGenerator = { 
		...
		saveForReply : function(aMessageURI, aReplyType) 
		{
			var info = this.generateForReply(
					...
					this.myAddress,
					(
						MessagesHeaderReader.getHeaderValueFrom('reply-to', aMessageURI) ||
